...
def createuserwin():
    def adam():
        ...
        if conad:
            cursor = conad.cursor()
            qry = "INSERT INTO starmoversdb.users (id, name, password, usertype) VALUES (%s, %s, %s, %s)"
            id = e1.get()
            name = e2.get()
            password = e3.get()
            usertype = e4.get()
            cursor.execute(qry, [id, name, password, usertype])
            conad.commit()
    ...
    Button(root, text="INSERT", command=adam, height=2, width=10).place(x=10, y=130)
    root.mainloop()
